...
function Avatar({ name, color }: { name: string; color: string }) {
  ...
  return <div className={"w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 "+color}>{initials}</div>;
}
...
export default function AdminUsersPage() {
  ...
  const openCreate = () => { setForm({...EMPTY, role: roleFilter}); setModal("create"); };
  ...
  const isBen = roleFilter === "BENEFICIARIO";
  const isForm = roleFilter === "FORMADOR";

  return (
    <AppShell allowedRoles={["ADMIN"]}>
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="font-display text-3xl font-bold text-text-primary">Gestion de Usuarios</h1><p className="text-text-secondary mt-1">{total} usuarios registrados</p></div>
          <div className="flex gap-3">
            <button onClick={openCreate} className="btn-primary flex items-center gap-2"><Plus className="w-4 h-4"/> Nuevo usuario</button>
            <button onClick={exportCSV} className="btn-outline flex items-center gap-2 text-sm"><Download className="w-4 h-4"/> CSV</button>
          </div>
        </div>

        {msg.text&&<div className={clsx("flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium border",msg.type==="err"?"bg-red-50 border-red-200 text-red-700":"bg-green-50 border-green-200 text-green-700")}>{msg.type==="err"?<XCircle className="w-4 h-4 flex-shrink-0"/>:<CheckCircle className="w-4 h-4 flex-shrink-0"/>}{msg.text}<button onClick={()=>setMsg({text:"",type:""})} className="ml-auto"><X className="w-4 h-4"/></button></div>}

        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400"/><input className="input pl-12 h-12 text-sm rounded-2xl" placeholder="Buscar por nombre, cedula o email..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}/></div>
          <div className="flex gap-2">
            {["BENEFICIARIO","FORMADOR","ADMIN"].map(r=>(
              <button key={r} onClick={()=>{setRoleFilter(r);setPage(1);}} className={clsx("px-5 py-2.5 rounded-2xl text-sm font-semibold transition-all",roleFilter===r?"bg-primary text-white shadow-brand":"bg-white border border-gray-200 text-text-secondary hover:border-primary hover:text-primary")}>
              ...
              return (
                <div key={u.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all duration-200 p-4">
                  <div className="flex items-center gap-4">
                    <Avatar name={u.firstName+" "+u.lastName} color={AVATAR_COLORS[i%AVATAR_COLORS.length]}/>
                    <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-4 items-center">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {u.gender&&<span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium",u.gender==="FEMENINO"?"bg-pink-50 text-pink-700":u.gender==="MASCULINO"?"bg-blue-50 text-blue-700":"bg-purple-50 text-purple-700")}>{u.gender==="FEMENINO"?"Mujer":u.gender==="MASCULINO"?"Hombre":"LGBTIQ+"}</span>}
                        {ct?<span className={"inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium "+cc.bg+" "+cc.text}><span className={"w-1.5 h-1.5 rounded-full "+cc.dot}/>{ct}</span>:<span className="text-xs text-gray-400 italic">Sin curso</span>}
                      </div>
                      <div className="flex items-center gap-2">
                        {u.isActive?<span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700"><span className="w-1.5 h-1.5 rounded-full bg-green-500"/>Activo</span>:<span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500"><span className="w-1.5 h-1.5 rounded-full bg-gray-400"/>Inactivo</span>}
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      <button onClick={()=>openCourse(u)} title="Asignar curso" className="w-9 h-9 rounded-xl hover:bg-green-50 text-green-600 flex items-center justify-center transition-colors"><BookOpen className="w-4 h-4"/></button>
                      ...
                      {isBen&&cid&&(completed?(
                        <>
                          <button onClick={()=>dlCert(u.id,cid,u.cedula)} className="flex items-center gap-1 bg-primary text-white px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-primary-dark transition-colors"><Award className="w-3.5 h-3.5"/>PDF</button>
                          <button onClick={()=>revokeCert(u.id,cid)} className="flex items-center gap-1 bg-orange-100 text-orange-700 px-2 py-1.5 rounded-xl text-xs font-semibold hover:bg-orange-200 transition-colors">Revocar</button>
                        </>
                      ):(
                        <button onClick={()=>enableCert(u.id,cid)} className="flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1.5 rounded-xl text-xs font-semibold hover:bg-green-200 transition-colors"><CheckCircle className="w-3.5 h-3.5"/>Habilitar</button>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        {total>20&&<div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} className="w-9 h-9 rounded-xl border border-gray-200 flex items-center justify-center hover:bg-gray-50 disabled:opacity-40"><ChevronLeft className="w-4 h-4"/></button>
            <span className="px-3 py-1.5 text-sm bg-gray-100 rounded-xl text-text-secondary">Pag. {page} de {totalPages}</span>
            <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page>=totalPages} className="w-9 h-9 rounded-xl border border-gray-200 flex items-center justify-center hover:bg-gray-50 disabled:opacity-40"><ChevronRight className="w-4 h-4"/></button>
          </div>
        </div>}
      </div>

      {modal&&(
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={e=>{if(e.target===e.currentTarget)setModal(null);}}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white flex items-center justify-between px-6 py-4 border-b border-gray-100 rounded-t-3xl">
              ...
              <button onClick={()=>setModal(null)} className="w-8 h-8 rounded-xl hover:bg-gray-100 flex items-center justify-center"><X className="w-4 h-4"/></button>
            </div>
            <div className="p-6 space-y-4">
              {(modal==="create"||modal==="edit")&&(
                <>
                  ...
                  {modal==="create"&&<div><label className="block text-xs font-bold text-text-muted mb-1.5">Contrasena</label><input className="input text-sm" type="password" value={form.password||""} onChange={e=>F("password",e.target.value)} placeholder="Dejar vacio = clave por defecto"/></div>}
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-xs font-bold text-text-muted mb-1.5">Genero</label>
                      <select className="input text-sm" value={form.gender||"FEMENINO"} onChange={e=>F("gender",e.target.value)}>
                        <option value="FEMENINO">Femenino</option><option value="MASCULINO">Masculino</option><option value="NO_BINARIO">LGBTIQ+</option>
                      </select>
                    </div>
                  ...
                  {/* Grupo poblacional SOLO para beneficiarios */}
                  ...
                  {/* Curso para beneficiarios Y formadores */}
                  {(form.role==="BENEFICIARIO"||form.role==="FORMADOR")&&(
                    <div><label className="block text-xs font-bold text-text-muted mb-1.5">Curso asignado</label>
              ...
              {modal==="password"&&(
                <>
                  <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-3 text-sm text-yellow-700">Cambiando contrasena de <strong>{selUser?.firstName} {selUser?.lastName}</strong></div>
                  <div><label className="block text-xs font-bold text-text-muted mb-1.5">Nueva contrasena</label><input className="input" type="password" value={form.password||""} onChange={e=>F("password",e.target.value)} placeholder="Min. 6 caracteres"/></div>
              ...
              {modal==="delete"&&(
                <>
                  ...
                  <button onClick={()=>doDelete(false)} disabled={saving} className="w-full bg-yellow-50 border border-yellow-200 text-yellow-700 hover:bg-yellow-100 rounded-2xl py-3 text-sm font-semibold flex items-center justify-center gap-2"><XCircle className="w-4 h-4"/> Desactivar</button>
                  <button onClick={()=>doDelete(true)} disabled={saving} className="w-full bg-red-50 border border-red-200 text-red-700 hover:bg-red-100 rounded-2xl py-3 text-sm font-semibold flex items-center justify-center gap-2"><Trash2 className="w-4 h-4"/> Eliminar permanentemente</button>
                </>
              )}
            </div>
          </div>
