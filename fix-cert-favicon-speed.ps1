Write-Host "=== FIX FAVICON + CERTIFICADO + VELOCIDAD ===" -ForegroundColor Yellow

# ── 1. FAVICON: copiar logo.png a public/ ────────────────────
$src = "frontend\public\images\logo.png"
if (Test-Path $src) {
  Copy-Item $src "frontend\public\favicon.png" -Force
  Copy-Item $src "frontend\public\favicon.ico" -Force
  Write-Host "Favicon copiado OK" -ForegroundColor Green
} else {
  Write-Host "AVISO: No se encontro logo.png" -ForegroundColor Yellow
}

# ── 2. LAYOUT: favicon en metadata ───────────────────────────
...
'
# Solo actualizar si tiene el contenido de metadata
$layout = [System.IO.File]::ReadAllText("$PWD\frontend\src\app\layout.tsx", [System.Text.Encoding]::UTF8)
if ($layout -notmatch "favicon.png") {
  $layout = $layout -replace "title:.*`"Buskando Parche.*`"", 'title: "Buskando Parche LMS - Kennedy",
    description: "Plataforma de formacion - Kennedy, Bogota",
    icons: { icon: "/favicon.png", shortcut: "/favicon.png", apple: "/favicon.png" }'
  $utf8NoBom = New-Object System.Text.UTF8Encoding $false
  [System.IO.File]::WriteAllText("$PWD\frontend\src\app\layout.tsx", $layout, $utf8NoBom)
  Write-Host "layout.tsx con favicon OK" -ForegroundColor Green
}

# ── 3. CERTIFICADO: ruta corregida ──────────────────────────
# En Docker: /app/src/routes/certificates.js
# __dirname = /app/src/routes
# ../../cert-templates = /app/cert-templates  ← CORRECTO
...
'
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllText("$PWD\backend\src\routes\certificates.js", $certRoute, $utf8NoBom)
Write-Host "certificates.js con ruta correcta OK" -ForegroundColor Green

# ── 4. DOCKERFILE FRONTEND: modo produccion (mas rapido) ────
...
'
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllText("$PWD\frontend\Dockerfile.prod", $dockerfile, $utf8NoBom)
Write-Host "Dockerfile.prod modo produccion OK" -ForegroundColor Green
...
Write-Host "2. Summary: 'Fix favicon certificado velocidad'" -ForegroundColor White
Write-Host "3. Commit to main -> Push origin" -ForegroundColor White
Write-Host ""
Write-Host "En el VPS:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "NOTA: El build de produccion demora mas la primera vez" -ForegroundColor Yellow
Write-Host "pero despues la app carga MUCHO mas rapido" -ForegroundColor Yellow
